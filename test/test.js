var unalib = require('../unalib/index');
 ...
 describe('unalib', function(){
    ...
    it('debería devolver true para URLs de imágenes', function(){
      assert.equal(unalib.is_valid_image_url('https://example.com/image.jpg'), true);
    });
    it('debería devolver false para URLs que no son imágenes', function(){
      assert.equal(unalib.is_valid_image_url('https://example.com/file.pdf'), false);
      assert.equal(unalib.is_valid_image_url('https://example.com/file.text'), false);
    });
  });

  describe('función is_valid_video_url', function(){
    it('debería devolver true para URLs de videos', function(){
      assert.equal(unalib.is_valid_video_url('https://example.com/video.mp4'), true);
    });
    it('debería devolver false para URLs que no son videos', function(){
      assert.equal(unalib.is_valid_video_url('https://example.com/file.jpg'), false);
    });
  });
